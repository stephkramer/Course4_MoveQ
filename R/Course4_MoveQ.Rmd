...
save(dat_anim_na, file = here('output/data-proc/tag5334_gps_proc.RData'))

# interchange file format .csv
write.csv(dat_anim_na, file = here('output/data-proc/tag5334_gps_proc.csv'))
```

Check your output folder for these files. The efficient '.RData' file storage
is about 4 times smaller than the '.csv' file. 
...
A nicer interactive plot with leaflet

```{r}

base_wgs84_x <- dat_anim_na$longitude[1]
base_wgs84_y <- dat_anim_na$latitude[1]

m_leaf <- leaflet(dat_anim_na) %>% 
  addTiles() %>%
  ...
  addCircles(lng = ~longitude, lat = ~latitude,~5,
             ...
             fillColor= 'blue',
             fillOpacity = 0.3) 
m_leaf # leaflet is bad for export -> create them in ggmap instead!

# no background: leaflet(dat_anim_na) %>% addCircles(lng = ~longitude, lat = ~latitude)
...
an sf/sp-object. Recap for styling the plot:
