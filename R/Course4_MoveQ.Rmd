...
### Extract basic movement metrics
...
Turning angles (makes only sense with high resolution data) and step length:
```{r}
...
###divide into months and do HR by months

```{r}
### MCP
cp <- mcp(anim_sp[,2], percent=95) # Minimum Convex Polygon (95%) per TierID # set unout for units!
plot(cp)
plot(anim_sp, add=TRUE, col= 'red')
cp # angular units because based on lat/ long -> does not make sense! Reproject data!

# project coordinate system in metres to calculate HR size
anim_sp_5243 <-  spTransform(anim_sp, CRS("+init=epsg:5243"))
anim_sf_5243 <-  st_transform(anim_sf, 5243)
cp2 <- mcp(anim_sp_5243[,2], percent=95) # Minimum Convex Polygon (95%) per TierID # set unout for units!
plot(cp2)
plot(anim_sp_5243, add=TRUE, col= 'red')
cp2
gArea(cp2)/1e6  # now in square-km
hrs <- mcp.area(anim_sp_5243[,2], percent=seq(50, 100, by = 5),unout='km2')
hrs # home range size in km2 for the different MCP-levels
writeOGR(cp, dsn=FileDir, "My_MCP95",'ESRI Shapefile',overwrite=T) #save as ESRI shapefile


### Kernel utility density
kud <- kernelUD(anim_sp_5243[,2], h="href")         # calculate kernel with h="href", animal identifier is second column
...
#  writeOGR(kud1[[1]], dsn=FileDir, "kernel_hr1",'ESRI Shapefile',overwrite=T)
#  writeOGR(vud[[1]], dsn=FileDir, "kernel_hr2",'ESRI Shapefile',overwrite=T)
...
animal$individual.local.identifier <- animal$tag.serial.number
animal$timestamp <- animal$timestamp.of.fix
animal$location.long <- animal$longitude
animal$location.lat <- animal$latitude


#ctmm_anim <- as.telemetry(animal)
ctmm_anim <- as.telemetry(animal[1:1000,]) #I am restricting the data to the first points for saving computation time
plot(ctmm_anim) #how strange! try:
help(plot.telemetry)
ctmm::plot(ctmm_anim, pch=17,lwd=0.1,cex=0.00001,xlim= c(-2000,2000)) #something does not work here....

# please do the following steps for fitting the best model (movement process) to your data
# https://cran.r-project.org/web/packages/ctmm/vignettes/variogram.html

SVF <- variogram(ctmm_anim)
plot(SVF,fraction=0.008) #Achtung, hectares !
title("zoomed in")
