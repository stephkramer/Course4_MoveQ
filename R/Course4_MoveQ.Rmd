...
└── Course4_MoveQ  (root folder)
    ...
    └ MyCourse.Rproj (the RStudio-Project file location)
```
...
I recommend that you store the cleaned file and continue with
the processed file after major data manipulations have been conducted to minimize
errors.
...
# TODO heatmap /activity plot for weeks~hrs (makes only sense for 
# yearly data)
```
...
hist(steplength)
max(steplength) # ~ 1 km in 20 min = 3 km / h

hist(speed(my_fox)) # units? -> check ?speed
```

## Spatial aggregation - the home-range or activity range concept


TODO def home range, MCP etc.
...
mcp_month
```
Note that there is a activity range for October (month 10), but the animal was only
collared  30.10.! This definitely does not make sense.
<br>
